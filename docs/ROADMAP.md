...
| P3 Launch moments | **Arrival certificate `shipped` 2026-09-23** · first-night mode · socialization passport · vaccine autopilot · "ate a grape" · sitter mode · gotcha day · renewal-explained (flagged) · FitnessProvider adapter + simulated provider | in progress — building in SPEC §6 order; everything here is buildable without an external dependency except the nearest-open-ER lookup in "ate a grape", which needs a Places API key |
...
- **SPEC §1 says "75+ tests".** It is 289 unit plus 94 emulator plus nine end-to-end scripts.
