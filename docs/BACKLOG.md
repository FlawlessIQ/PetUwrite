...
| **U8** | **UAT run 2's three** (UAT-PLAN §8). **N3, Sev 2:** after "Check it" the urgent answer is not scrolled into view or announced (`aria-live`) — on a phone with the keyboard up it may be hidden. **N1, Sev 3:** with nothing answered, Home says "doing well on everything we can see" for a puppy we cannot see, then "Ask the companion about it" with no "it". **N2, Sev 4:** never-asked review questions still say "since last year" | Claude Code | S | — |
