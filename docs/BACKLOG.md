...
| **U4** | **The Remember pass leaks on four more tabs** (UAT run 1, D19, Sev 1). After a death: Life still says "on track for 12.9–15.2 healthy years"; Rewards reports "+130 this week, from care streaks and activity"; Care keeps the Ask box and a scripted "Should I be worried?" thread; Coverage offers "Take this cover for Bruno". Same fix shape as U1 — enforce in the engines, extend `verify-remember` to every tab | Claude Code | S | — |
| **U5** | **The collapse screen has no way to find a vet** (D17, Sev 2). "Stop and ring a vet now" offers only poison lines; "Find the nearest vet open now" exists only on #/ate | Claude Code | S | — |
| **U6** | **Defaults presented as answers** (D2, D3, D18). The levers say "Set to what you told us" over unasked defaults; Home builds its biggest lever on one (dental "Weekly", for a 9-week-old); the medication form preselects "Twice a day" | Claude Code | S | — |
| **U7** | **Run 1's copy and small defects** — D1, D4–D16 except those above: an honesty set (D9, D11, D12, D13), one for counsel (D14), the customer-visible "VET-REVIEW:" tag (D16), the review's Done button (D7, D8), UK examples (D5), and four cosmetic ones. Full list in UAT-PLAN §8 | Claude Code (+L for D14) | M | — |
