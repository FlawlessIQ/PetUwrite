...
|---|---|---|---|---|---|---|---|
| **D19** | A8.5 | After "Bruno has died": **Life** still says "Bruno is on track for 12.9–15.2 healthy years"; **Rewards** "973 points · +130 this week, from care streaks and activity", streaks, Redeem; **Care** keeps the Ask box and the scripted "Should I be worried?… Worth a vet's eyes"; **Coverage** "Take this cover for Bruno" with a monthly price. `verify-remember` covers only Life's offers, Home, Shop and the Health File | Nothing asks, suggests, sells or reports activity | **1** | — | Claude | fixed on `uat-fixes-2` (U4) |
| **D17** | A5.2 | "Stop and ring a vet now" (collapse, pale gums) offers only three poison lines. "Find the nearest vet open now" exists on #/ate but not here | A way to find an emergency vet on the most urgent screen | **2** | — | Claude | fixed on `uat-fixes-2` (U5) |
