...
class VetHistoryParser {
  ...
  Future<VetRecordData> parseUploadedPdfForCase({
    ...
    try {
      final storageRef = await _uploadToCaseStorage(pdfFile, caseId, filename, petId: petId);
      final downloadUrl = await storageRef.getDownloadURL();

      final extractedText = await _extractTextFromPdf(downloadUrl);

      // Store extracted text in Storage (avoid Firestore doc size limits)
      final textRef = await _uploadExtractedText(caseId: caseId, sourceFilename: filename, extractedText: extractedText);
      final extractedTextUrl = await textRef.getDownloadURL();

      final parsedData = await _parseWithAI(extractedText);
      final confidence = _estimateConfidence(extractedText: extractedText, parsedData: parsedData);
      ...
      // Low confidence -> referral flag (lane management, not a coverage decision).
      if (confidence < referralConfidenceThreshold) {
        await _firestore.collection('underwriting_cases').doc(caseId).update({
          'status': 'referred',
          'updatedAt': FieldValue.serverTimestamp(),
        });
        await _firestore.collection('underwriting_cases').doc(caseId).collection('events').add({
      ...
      throw VetHistoryParseException('Failed to parse uploaded PDF for case: $e');
    }
  }
  ...
  Future<Reference> _uploadToCaseStorage(
    ...
    final fileName = filename ?? 'vet_record_${DateTime.now().millisecondsSinceEpoch}.pdf';
    final storageRef = _storage.ref().child('vet_records/cases/$caseId/$fileName');
  ...
  Future<Reference> _uploadExtractedText({
    ...
    final base = (sourceFilename ?? 'vet_record').replaceAll('.pdf', '');
    final textName = '${base}_${DateTime.now().millisecondsSinceEpoch}.txt';
    final ref = _storage.ref().child('vet_records/cases/$caseId/extracted/$textName');
