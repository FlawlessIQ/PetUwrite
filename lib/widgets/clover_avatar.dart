...
/// Clover - The Empathetic AI Pet Insurance Assistant
/// 
...
class CloverAvatar extends StatefulWidget {
  final CloverExpression expression;
  final CloverState state;
  final double size;
  ...
  const CloverAvatar({
    super.key,
    this.expression = CloverExpression.happy,
    this.state = CloverState.idle,
    this.size = 120,
    this.message,
    this.animated = true,
    this.showGlow = false,
  ...
  @override
  State<CloverAvatar> createState() => _CloverAvatarState();
}

/// Clover's animation states
enum CloverState {
...
class _CloverAvatarState extends State<CloverAvatar>
  ...
  void initState() {
    ...
    if (widget.showGlow || widget.state == CloverState.typing) {
  ...
  @override
  void didUpdateWidget(CloverAvatar oldWidget) {
    super.didUpdateWidget(oldWidget);
    
    // Update glow based on state
    if (widget.state == CloverState.typing || widget.showGlow) {
      ...
      children: [
        AnimatedBuilder(
          ...
          builder: (context, child) {
            ...
            final glowIntensity = (widget.state == CloverState.typing || widget.showGlow)
        ...
        if (widget.state == CloverState.typing) ...[
  ...
  List<Color> _getGradientColors() {
    switch (widget.expression) {
      case CloverExpression.happy:
        return [const Color(0xFF667eea), const Color(0xFF764ba2)];
      case CloverExpression.thinking:
        return [const Color(0xFF4facfe), const Color(0xFF00f2fe)];
      case CloverExpression.empathetic:
        return [const Color(0xFFf093fb), const Color(0xFFF5576C)];
      case CloverExpression.celebrating:
        return [const Color(0xFFfa709a), const Color(0xFFfee140)];
      case CloverExpression.concerned:
        return [const Color(0xFF8e9eab), const Color(0xFFeef2f3)];
      case CloverExpression.working:
        return [const Color(0xFF667eea), const Color(0xFF764ba2)];
    }
  }
...
/// Custom painter for Clover's face
class _CloverPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(covariant _CloverPainter oldDelegate) {
    return oldDelegate.expression != expression || oldDelegate.blinkAmount != blinkAmount;
  }
}

/// Clover's expressions for different contexts
enum CloverExpression {
