 ...
 * Automatically flags high-risk quotes (score > 80) as automation exceptions.
 */
exports.flagHighRiskQuote = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      // Create notification for admins. Keep legacy fields for compatibility.
      await admin
        .firestore()
        ...
        .add({
          ...
          ownerName:
            `${quoteData.owner?.firstName || ""} ${quoteData.owner?.lastName || ""}`.trim(),
          aiDecision: quoteData.riskScore?.aiAnalysis?.decision || "Unknown",
          targetRole: 2, // Legacy admin/underwriter role.
          read: false,
      ...
      console.log(`Found ${underwriters.size} admin operators to notify`);

      // Send notifications to each underwriter (optional - requires FCM tokens)
      const notificationPromises = underwriters.docs.map(
        async (underwriterDoc) => {
          ...
          if (underwriterData.fcmToken) {
            return admin.messaging().send({
              ...
              notification: {
                title: "⚠️ Quote Automation Exception",
                body: `${quoteData.pet?.name} (Risk: ${riskScore}) needs an evidence or eligibility check`
              },
              data: {
                type: "high_risk_quote",
                quoteId: quoteId,
      ...
      console.log(
        `Successfully flagged quote ${quoteId} and notified admin operators`
      );
      return { success: true };
    } catch (error) {
...
/**
 * Triggered when a quote is updated with the legacy humanOverride field.
 * Logs the audited exception override and updates statistics.
 */
exports.onQuoteOverride = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    // Check if an exception override was added.
    if (!beforeData.humanOverride && afterData.humanOverride) {
      console.log(`Quote ${quoteId} received an exception override`);
          ...
          if (quoteDoc.exists) {
            ...
            if (quoteCreatedAt && overrideTimestamp) {
              const responseTimeHours =
                (overrideTimestamp - quoteCreatedAt) / (1000 * 60 * 60);
...
exports.alertPendingQuotes = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      console.log(
        `Found ${quotesWithoutOverride.length} pending quotes > 4 hours old`
      );
          ...
          if (underwriterData.fcmToken) {
            return admin.messaging().send({
              ...
              notification: {
                ...
                body: `${quotesWithoutOverride.length} quotes need automation exception resolution`
              },
              data: {
...
exports.getOverrideAnalytics = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    overrides.forEach((doc) => {
      ...
      // Track automated vs exception-control decisions.
    ...
    if (overrides.size > 0) {
      ...
      analytics.overrideRate =
        ((overrideApprovalCount - aiApprovalCount) / overrides.size) * 100;
    }
