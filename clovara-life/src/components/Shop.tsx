  ...
  if (isRemembered(pet)) {
    ...
    return (
      ...
      <div className="mx-auto w-full max-w-shell px-5 pb-24 pt-8">
        ...
        <p className="mt-2 max-w-[52ch] text-lead leading-relaxed text-ink-2">
  ...
  return (
    <div className="mx-auto w-full max-w-shell px-5 pb-24 pt-8 sm:pt-10">
      <header className="mb-6">
        ...
        <p className="mt-2 max-w-[64ch] text-lead leading-relaxed text-ink-2">
          ...
          {picked.map((p) => {
            ...
            return (
              <article key={p.id} className="card flex flex-col overflow-hidden p-0">
                <div className="relative flex h-[112px] items-end justify-center bg-sage/70 pb-3 text-display-xl">
                  <span aria-hidden="true" className="leading-none">{p.emoji}</span>
                  <span className="absolute left-3 right-3 top-3 truncate rounded-full bg-white/92 px-2.5 py-1 text-caption font-medium text-deep">
                    {p.why}
                  </span>
                </div>

                <div className="flex flex-1 flex-col p-4">
                  <h3 className="text-lead font-medium leading-snug text-ink">{p.name}</h3>
                  <p className="mt-1 text-body-sm leading-snug text-ink-2">{p.subtitle}</p>

                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <span className={`rounded-full px-2.5 py-0.5 text-caption font-medium ${STRENGTH_STYLE[p.claimStrength]}`}>
                  ...
                  <div className="mt-auto pt-4">
                    <div className="flex items-baseline gap-2">
                      {free ? (
                        <span className="font-display text-heading-sm font-semibold text-forest">Free</span>
                      ) : (
                        <>
                          <span className="font-display text-heading-sm font-semibold text-forest">
                            ${member ? p.memberPrice : p.price}
                          </span>
                          {member ? (
                            <>
                              <span className="text-body-sm text-ink-2 line-through">${p.price}</span>
                              <span className="text-body-sm text-ink-2">member</span>
                            </>
                          ) : (
                    ...
                    <p className="mt-0.5 text-body-sm text-ink-2">Earns {p.points} points</p>

                    <button
                      type="button"
                      onClick={() => setOpen(expanded ? null : p.id)}
                      aria-expanded={expanded}
                      className="mt-3 text-body-sm text-forest hover:text-deep text-action"
                    >
                      {expanded ? 'Hide the evidence' : 'What the evidence says'}
                    </button>
                    {expanded && (
                      <p className="reveal mt-2 rounded-soft bg-cream/70 p-3 text-body-sm leading-relaxed text-ink/80">
                        {p.evidenceNote}
                      </p>
                    )}
                  </div>
          ...
          <ul className="card divide-y divide-line p-0">
            {staples.map((p) => (
              <li key={p.id} className="flex items-center gap-4 px-5 py-3.5">
                <span aria-hidden="true" className="grid h-11 w-11 shrink-0 place-items-center rounded-soft bg-cream text-heading">
                  {p.emoji}
                </span>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-body-lg font-medium text-ink">{p.name}</p>
                  <p className="truncate text-body-sm text-ink-2">{p.subtitle}</p>
                </div>
                <span className="shrink-0 text-body-lg font-medium text-forest">
