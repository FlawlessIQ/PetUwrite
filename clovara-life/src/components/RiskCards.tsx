  ...
  return (
    <section className="card overflow-hidden" aria-labelledby="risks-heading">
      <div className="border-b border-line bg-cream/50 px-5 py-4 sm:px-6">
        <h2 id="risks-heading" className="font-display text-heading-lg leading-tight text-ink">
          ...
          return (
            <li key={card.id} className="px-5 py-4 sm:px-6">
              <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1.5">
                <h3 className="text-lead font-medium leading-snug text-ink">{card.name}</h3>
                <span
                  className={`shrink-0 rounded-full px-2.5 py-0.5 text-body-sm font-semibold ${mode.className}`}
                >
                  {mode.label}
                </span>
              </div>

              <p className="mt-1 text-body-sm font-medium text-forest">{card.window}</p>

              <p className="mt-2 text-body-lg leading-relaxed text-ink/80">
                <span className="text-ink-2">What you might notice — </span>
                {card.watch}
              </p>
              <p className="mt-1.5 text-body-lg leading-relaxed text-ink/80">
                <span className="text-ink-2">What to do — </span>
                {card.action}
              </p>

              {card.context && (
                <p className="mt-2.5 rounded-soft border border-line bg-cream/60 px-3 py-2.5 text-body leading-relaxed text-ink/80">
                  <span className="mb-1 block text-caption font-semibold uppercase tracking-[0.18em] text-ink-2">
                    About {name} specifically
                  </span>
                  {card.context.text}
                  <span className="mt-1.5 block text-body-sm text-ink-2">
      ...
      <p className="border-t border-line bg-cream/50 px-5 py-3.5 text-body-sm leading-relaxed text-ink-2 sm:px-6">
