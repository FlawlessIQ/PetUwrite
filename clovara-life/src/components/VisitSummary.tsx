  ...
  return (
    <section className="card overflow-hidden" aria-labelledby="visit-heading">
      <div className="border-b border-line bg-cream/50 px-5 py-4">
        ...
        <h2 id="visit-heading" className="mt-1 font-display text-heading text-ink">
      ...
      <div className="px-5 py-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" className="pill-primary px-5 py-2.5 text-body-lg" onClick={copy}>
          ...
          <div className="mt-5 space-y-5">
            {summary.sections.map((s) => (
              <div key={s.heading}>
                ...
                {s.lines.length === 0 ? (
                  <p className="mt-1.5 text-body leading-relaxed text-ink-2">{s.emptyNote}</p>
                ) : (
                  <ul className="mt-1.5 divide-y divide-line border-t border-line">
                    {s.lines.map((l) => (
                      <li key={l.label} className="py-2">
                        <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-0.5">
                          <span className="text-body-lg text-ink">{l.label}</span>
                          <span className={`text-body-sm ${TOLD_STYLE[l.told]}`}>{l.told}</span>
                        </div>
                        <p className="mt-0.5 text-body leading-relaxed text-ink-2">{l.value}</p>
                      </li>
                    ))}
                  </ul>
                )}
            ...
            <div>
              ...
              <ul className="mt-1.5 space-y-1.5">
                {summary.caveats.map((c) => (
                  <li key={c} className="text-body-sm leading-relaxed text-ink-2">
                    {c}
                  </li>
                ))}
              </ul>
